...
def cargar_coordenadas():
    coordenadas = []
    for i in range(4):
        latitud = float(input(f"Ingrese la latitud de la cámara {i + 1}: "))
        longitud = float(input(f"Ingrese la longitud de la cámara {i + 1}: "))
...
def filtrar_hemisferio(coordenadas):
    ...
    for lat, lon in coordenadas:
        if lat > 0:
            count += 1
    print(f"Cantidad de cámaras en el hemisferio norte: {count}")
