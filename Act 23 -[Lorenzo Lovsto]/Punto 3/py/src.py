...
def ordenar(equipos, puntos, diferencia):
    ...
    for i in range(len(puntos)-1):
        ...
        for j in range(i+1, len(puntos)):
            ...
            if puntos[j] > puntos[i] or (puntos[j] == puntos[i] and diferencia[j] > diferencia[i]):

                puntos[i], puntos[j] = puntos[j], puntos[i]
                diferencia[i], diferencia[j] = diferencia[j], diferencia[i]
                equipos[i], equipos[j] = equipos[j], equipos[i]

    print("Tabla:")

    for i in range(len(equipos)):
        print(i+1, equipos[i], puntos[i], diferencia[i])
